...
class CartService {
      static async addCart(req: IRequestCustom) {
            ...
            const foundCart = await cartModel.findOne({ cart_product_id: product_id }).populate('cart_product_id')
            ...
            const addCart = await cartModel.findOneAndUpdate(
                  { cart_product_id: new Types.ObjectId(product_id), cart_user_id: new Types.ObjectId(user?._id) },
                  {
                        $set: {
                              cart_product_id: product_id,
      ...
      static async getMyCart(req: IRequestCustom) {
            const { user } = req
            const foundCart = await cartModel.find({ cart_user_id: user?._id }).populate({
